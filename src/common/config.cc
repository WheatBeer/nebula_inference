...
void section_config_t::add_setting(std::string m_key, std::string m_value) {
    settings.insert(std::make_pair(m_key, m_value));
}

// Check if a setting exists.
bool section_config_t::exists(std::string m_key) {
    return settings.find(m_key) != settings.end();
}
...
void config_t::parse(std::string m_config_name) {
    ...
    if(!file_stream.is_open()) {
		std::cerr << "Error: failed to open " << m_config_name << std::endl; exit(1);
    }
    ...
    while(getline(file_stream, line)) {
        ...
        if(line[0] == '[') {
            ...
            if(eq == std::string::npos) {
				std::cerr << "Error: invalid config" << std::endl << line << std::endl; exit(1);
            }
